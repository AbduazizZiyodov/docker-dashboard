...
@router.get("", response_model=t.List[DockerImageResponse])
...
@router.get("/search/", response_model=t.List[DockerImageSearchResult])
async def search_image(term: str, limit: t.Optional[int] = 10):
    """Search for images on Docker Hub. Similar to the ``docker search``
    command."""
    return client.images.search(term=term, limit=limit)
